...
from legacy_slam_support import (
    LegacyBrakeCalibrator,
    LegacyLoiterObserver,
    external_imu_attitude,
    gps_input_time_fields,
)
...
class MonitorState:
    ...
    gps_raw_int: Optional[object] = None
...
FLOW_CSV_FIELDS = [
    ...
    "external_imu_present",
    "external_imu_roll_deg",
    "external_imu_pitch_deg",
    "external_imu_yaw_deg",
]
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help=(
            "Repo-local folder base for per-flight sensor CSV logs. If the value "
            "ends in .csv, its stem becomes the folder name. Each armed flight "
            "writes a fresh file named N_YYYYMMDD-HHMMSS.csv inside that folder."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--disable-external-imu",
        action="store_true",
        help="Disable the new external IMU attitude assist and use FC attitude only.",
    )
    parser.add_argument("--imu-port", default="auto", help="External IM10A port, or auto.")
    parser.add_argument("--imu-baud", default="auto", help="External IM10A baud, or auto.")
    parser.add_argument("--imu-scan-seconds", type=float, default=0.8)
    parser.add_argument(
        "--external-imu-yaw-mode",
        choices=["fc", "external"],
        default="fc",
        help="Use external IMU roll/pitch; yaw comes from FC by default unless set to external.",
    )
    parser.add_argument(
        "--disable-loiter-observer",
        action="store_true",
        help="Disable GPS-assisted LOITER soft observation.",
    )
    parser.add_argument("--loiter-observer-log", default="logs/legacy_flow_loiter_observer.log")
    parser.add_argument("--loiter-observer-interval", type=float, default=20.0)
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Disable bounded LOITER-learned velocity scaling on the flow GPS2 feed.",
    )
    parser.add_argument("--legacy-profile-path", default="runtime/legacy_flow_calibration.json")
    parser.add_argument(
        "--disable-brake-calibration",
        action="store_true",
        help="Disable BRAKE-mode legacy calibration bookkeeping.",
    )
    parser.add_argument("--legacy-brake-duration-s", type=float, default=12.0)
    parser.add_argument("--legacy-brake-min-samples", type=int, default=80)
...
def request_vehicle_serial(master, timeout_s: float = 2.0) -> str:
    request_message_command = getattr(mavutil.mavlink, "MAV_CMD_REQUEST_MESSAGE", None)
    autopilot_version_msg_id = getattr(
        mavutil.mavlink,
        "MAVLINK_MSG_ID_AUTOPILOT_VERSION",
        None,
    )
    if request_message_command is not None and autopilot_version_msg_id is not None:
        master.mav.command_long_send(
            master.target_system,
            master.target_component,
            request_message_command,
            0,
            autopilot_version_msg_id,
            0,
            0,
            0,
            0,
        ...
        while time.time() <= deadline:
            ...
            if msg.get_type() != "AUTOPILOT_VERSION":
                continue
            uid = int(getattr(msg, "uid", 0) or 0)
            if uid > 0:
                return f"{uid:016X}"

    board_serial = request_parameter(master, "BRD_SERIAL_NUM", timeout_s=1.0)
    if board_serial is not None:
        board_serial_int = int(round(board_serial))
        if board_serial_int > 0:
            return str(board_serial_int)

    return f"SYS{int(getattr(master, 'target_system', 0) or 0)}"


def resolve_flow_csv_log_directory(base_path: str) -> str:
    ...
    directory_name = sanitize_filename_component(directory_name, "flight_sensor_logs")
    log_directory = os.path.join(repo_directory, directory_name)
    os.makedirs(log_directory, exist_ok=True)
    return log_directory


def next_flight_log_index(log_directory: str) -> int:
    next_index = 1
    for entry_name in os.listdir(log_directory):
        if not entry_name.lower().endswith(".csv"):
            continue
        index_text = entry_name.split("_", 1)[0]
        if index_text.isdigit():
            next_index = max(next_index, int(index_text) + 1)
    return next_index


def resolve_flow_csv_log_path(base_path: str, flight_index: int, run_id: str) -> str:
    log_directory = resolve_flow_csv_log_directory(base_path)
    safe_run_id = sanitize_filename_component(run_id, "UNKNOWN_TIME")
    return os.path.join(log_directory, f"{flight_index}_{safe_run_id}.csv")
...
def send_flow_gps_input(
    ...
    time_usec, gps_week, gps_week_ms = gps_input_time_fields(timestamp_us / 1e6)
...
def update_flow_external_nav(
    ...
    origin_lon_deg: Optional[float],
    origin_alt_m: Optional[float],
    soft_correction: Optional[LegacyLoiterObserver] = None,
):
    ...
    if not flow_health_is_ok:
        ...
        return
    if soft_correction is not None:
        velocity_ned = soft_correction.apply_velocity_ned(*velocity_ned)
...
def drain_messages(master, monitor: MonitorState, args, qgc_forwarder=None):
    ...
    while True:
        ...
        if msg_type == "LOCAL_POSITION_NED":
            ...
            monitor.gps_raw_int = msg
        elif msg_type == "GLOBAL_POSITION_INT":
            monitor.global_position_int = msg
...
def main():
    ...
    if args.legacy_brake_min_samples < 1:
        raise SystemExit("--legacy-brake-min-samples must be >= 1")
    ...
    external_imu_reader = None
    external_imu_sample = None
    external_imu_error_reported = False
    loiter_observer = LegacyLoiterObserver(
        enabled=not args.disable_loiter_observer,
        log_path=args.loiter_observer_log,
        message_interval_s=args.loiter_observer_interval,
    ...
    if args.disable_external_imu:
        ...
        try:
            external_imu_reader = Im10aReader.open(args.imu_port, args.imu_baud, args.imu_scan_seconds)
            announce_status(master, f"Legacy bridge external IMU active on {external_imu_reader.port}")
        except Exception as exc:  # noqa: BLE001
            announce_status(
                master,
                f"Legacy bridge external IMU unavailable: {exc}",
                mavutil.mavlink.MAV_SEVERITY_WARNING,
            )
    ...
    legacy_observer_summary = {"score": 0.0, "recommendation": "not started", "best_score": 0.0}
    ...
    vehicle_serial = sanitize_filename_component(request_vehicle_serial(master), "UNKNOWN_SERIAL")
    flow_csv_file = None
    flow_csv_writer = None
    flow_csv_path = None
    flow_csv_directory = None
    flight_log_index = 0
    current_flight_run_id = ""
    flight_log_start_time: Optional[float] = None
    next_csv_flush = time.time() + 1.0
    if not args.disable_flow_csv_log:
        try:
            flow_csv_directory = resolve_flow_csv_log_directory(args.flow_csv_log)
            print(f"Companion flight sensor log folder: {flow_csv_directory}")
        except OSError as exc:
            print(
                "Unable to prepare flight sensor log folder"
                f" {args.flow_csv_log}: {exc}"
            )
    ...
    try:
        while True:
            ...
            if external_imu_reader is not None:
                try:
                    sample = external_imu_reader.poll(duration_s=0.005)
                    if sample is not None:
                        external_imu_sample = sample
                        monitor.attitude = external_imu_attitude(
                            monitor.attitude,
                            external_imu_sample,
                            args.external_imu_yaw_mode,
                        )
                        external_imu_error_reported = False
                except Exception as exc:  # noqa: BLE001
                    if not external_imu_error_reported:
                        announce_status(
                            master,
                            f"Legacy bridge external IMU read failed: {exc}",
                            mavutil.mavlink.MAV_SEVERITY_WARNING,
                        )
                        external_imu_error_reported = True
            if (
            ...
            if (
                not args.disable_flow_csv_log
                and flow_csv_directory is not None
                and monitor.armed
                and flow_csv_writer is None
            ):
                try:
                    flight_log_index = next_flight_log_index(flow_csv_directory)
                    current_flight_run_id = time.strftime("%Y%m%d-%H%M%S", time.localtime(now))
                    flow_csv_path = resolve_flow_csv_log_path(
                        args.flow_csv_log,
                        flight_log_index,
                        current_flight_run_id,
                    )
                    flow_csv_file, flow_csv_writer = open_flow_csv_log(flow_csv_path)
                    flight_log_start_time = now
                    next_csv_flush = now + 1.0
                    print(f"Flight sensor CSV log: {flow_csv_path}")
                except OSError as exc:
                    print(
                        "Unable to open flight sensor CSV log"
                        f" {flow_csv_path or flow_csv_directory}: {exc}"
                    )
                    flow_csv_file = None
                    flow_csv_writer = None
                    flight_log_start_time = None

            if not monitor.armed and flow_csv_file is not None:
                try:
                    flow_csv_file.flush()
                    flow_csv_file.close()
                finally:
                    print(
                        "Closed flight sensor CSV log:"
                        f" {flow_csv_path or 'unknown'}"
                    )
                    flow_csv_file = None
                    flow_csv_writer = None
                    flow_csv_path = None
            ...
            update_flow_external_nav(
                ...
                origin_lon_deg,
                origin_alt_m,
                None if args.disable_legacy_live_soft_correction else loiter_observer,
            )
            if (
                (args.external_nav_from_flow or args.gps_input_from_flow)
            ...
            legacy_observer_summary = loiter_observer.update(
                master,
                monitor,
                flow_external_nav_state,
                flow,
            ...
            if flow_csv_writer is not None:
                ...
                gps_raw_int = monitor.gps_raw_int
                global_position_int = monitor.global_position_int
                scaled_pressure = monitor.scaled_pressure
                raw_imu = monitor.raw_imu
                try:
                    flow_csv_writer.writerow(
                        {
                            "flight_index": flight_log_index,
                            "vehicle_serial": vehicle_serial,
                            "run_id": current_flight_run_id,
                            "flight_started_s": csv_value(flight_log_start_time),
                            "unix_s": csv_value(now),
                            "run_s": csv_value(
                                now - flight_log_start_time
                                if flight_log_start_time is not None
                                else None
                            ),
                            ...
                            "range_orientation": (
                                monitor.rangefinder_orientation
                                if monitor.rangefinder_orientation is not None
                                else ""
                            ),
                            ...
                            "gps_fix_type": (
                                getattr(gps_raw_int, "fix_type", "")
                                if gps_raw_int is not None
                                else ""
                            ),
                            "gps_sats": (
                                getattr(gps_raw_int, "satellites_visible", "")
                                if gps_raw_int is not None
                                else ""
                            ),
                            "gps_lat_deg": csv_value(
                                (
                                    getattr(gps_raw_int, "lat", 0) / 1e7
                                    if gps_raw_int is not None
                                    and int(getattr(gps_raw_int, "lat", 0) or 0) != 0
                                    else None
                                )
                            ),
                            "gps_lon_deg": csv_value(
                                (
                                    getattr(gps_raw_int, "lon", 0) / 1e7
                                    if gps_raw_int is not None
                                    and int(getattr(gps_raw_int, "lon", 0) or 0) != 0
                                    else None
                                )
                            ),
                            "gps_alt_m": csv_value(
                                (
                                    getattr(gps_raw_int, "alt", 0) / 1000.0
                                    if gps_raw_int is not None
                                    and int(getattr(gps_raw_int, "alt", 0) or 0) != 0
                                    else None
                                )
                            ),
                            "gps_speed_m_s": csv_value(
                                (
                                    getattr(gps_raw_int, "vel", 0) / 100.0
                                    if gps_raw_int is not None
                                    and int(getattr(gps_raw_int, "vel", 0) or 0) != 65535
                                    else None
                                )
                            ),
                            "gps_course_deg": csv_value(
                                (
                                    getattr(gps_raw_int, "cog", 0) / 100.0
                                    if gps_raw_int is not None
                                    and int(getattr(gps_raw_int, "cog", 0) or 0) != 65535
                                    else None
                                )
                            ),
                            "gps_hdop": csv_value(
                                (
                                    getattr(gps_raw_int, "eph", 0) / 100.0
                                    if gps_raw_int is not None
                                    and int(getattr(gps_raw_int, "eph", 0) or 0) != 65535
                                    else None
                                )
                            ),
                            "gpos_lat_deg": csv_value(
                                (
                                    getattr(global_position_int, "lat", 0) / 1e7
                                    if global_position_int is not None
                                    and int(getattr(global_position_int, "lat", 0) or 0) != 0
                                    else None
                                )
                            ),
                            "gpos_lon_deg": csv_value(
                                (
                                    getattr(global_position_int, "lon", 0) / 1e7
                                    if global_position_int is not None
                                    and int(getattr(global_position_int, "lon", 0) or 0) != 0
                                    else None
                                )
                            ),
                            "gpos_alt_m": csv_value(
                                (
                                    getattr(global_position_int, "alt", 0) / 1000.0
                                    if global_position_int is not None
                                    else None
                                )
                            ),
                            "gpos_rel_alt_m": csv_value(
                                (
                                    getattr(global_position_int, "relative_alt", 0) / 1000.0
                                    if global_position_int is not None
                                    else None
                                )
                            ),
                            "gpos_vx_m_s": csv_value(
                                (
                                    getattr(global_position_int, "vx", 0) / 100.0
                                    if global_position_int is not None
                                    else None
                                )
                            ),
                            "gpos_vy_m_s": csv_value(
                                (
                                    getattr(global_position_int, "vy", 0) / 100.0
                                    if global_position_int is not None
                                    else None
                                )
                            ),
                            "gpos_vz_m_s": csv_value(
                                (
                                    getattr(global_position_int, "vz", 0) / 100.0
                                    if global_position_int is not None
                                    else None
                                )
                            ),
                            "gpos_heading_deg": csv_value(
                                (
                                    getattr(global_position_int, "hdg", 0) / 100.0
                                    if global_position_int is not None
                                    and int(getattr(global_position_int, "hdg", 0) or 0) != 65535
                                    else None
                                )
                            ),
                            "baro_press_abs_hpa": csv_value(
                                getattr(scaled_pressure, "press_abs", None)
                                if scaled_pressure is not None
                                else None
                            ),
                            "baro_press_diff_hpa": csv_value(
                                getattr(scaled_pressure, "press_diff", None)
                                if scaled_pressure is not None
                                else None
                            ),
                            "baro_temp_c": csv_value(
                                (
                                    getattr(scaled_pressure, "temperature", 0) / 100.0
                                    if scaled_pressure is not None
                                    else None
                                )
                            ),
                            "imu_xacc": csv_value(
                                getattr(raw_imu, "xacc", None) if raw_imu is not None else None
                            ),
                            "imu_yacc": csv_value(
                                getattr(raw_imu, "yacc", None) if raw_imu is not None else None
                            ),
                            "imu_zacc": csv_value(
                                getattr(raw_imu, "zacc", None) if raw_imu is not None else None
                            ),
                            "imu_xgyro": csv_value(
                                getattr(raw_imu, "xgyro", None) if raw_imu is not None else None
                            ),
                            "imu_ygyro": csv_value(
                                getattr(raw_imu, "ygyro", None) if raw_imu is not None else None
                            ),
                            "imu_zgyro": csv_value(
                                getattr(raw_imu, "zgyro", None) if raw_imu is not None else None
                            ),
                            "imu_xmag": csv_value(
                                getattr(raw_imu, "xmag", None) if raw_imu is not None else None
                            ),
                            "imu_ymag": csv_value(
                                getattr(raw_imu, "ymag", None) if raw_imu is not None else None
                            ),
                            "imu_zmag": csv_value(
                                getattr(raw_imu, "zmag", None) if raw_imu is not None else None
                            ),
                            "ekf_flags": (
                            ...
                            "external_imu_present": int(external_imu_sample is not None),
                            "external_imu_roll_deg": csv_value(
                                getattr(external_imu_sample, "roll_deg", None)
                                if external_imu_sample is not None
                                else None
                            ),
                            "external_imu_pitch_deg": csv_value(
                                getattr(external_imu_sample, "pitch_deg", None)
                                if external_imu_sample is not None
                                else None
                            ),
                            "external_imu_yaw_deg": csv_value(
                                getattr(external_imu_sample, "yaw_deg", None)
                                if external_imu_sample is not None
                                else None
                            ),
                        }
                    ...
                    print(f"Flight sensor CSV logging disabled after write error: {exc}")
                    try:
            ...
            if now >= next_status:
                summary = [
                    ...
                    f"legacy_score={legacy_observer_summary.get('score', 0.0):.1f}/10",
                    f"legacy_best={legacy_observer_summary.get('best_score', 0.0):.1f}/10",
