...
def test_default_config_is_conservative() -> None:
    ...
    assert config.lidar.ethernet_interface == "enP8p1s0"
    assert config.lidar.jetson_ip == "192.168.1.100"
    assert config.lidar.jetson_prefix_length == 24
...
def test_lidar_addresses_must_share_configured_subnet(
    tmp_path: Path,
) -> None:
    source = (ROOT / "config" / "system.yaml").read_text(encoding="ascii")
    source = source.replace(
    ...
    config_path = tmp_path / "invalid-lidar-network.yaml"
    config_path.write_text(source, encoding="ascii")
