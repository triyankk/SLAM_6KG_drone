#!/usr/bin/env bash
# Run:
#   sudo bash /home/atas/vscode/intellisense_slam/install/install_vio_flight_service.sh

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && cd .. && pwd)"
SENSOR_INSTALLER="${SCRIPT_DIR}/install/install_usb_serial_sensors_autostart.sh"
SERVICE_TEMPLATE="${SCRIPT_DIR}/hardware/configs/templates/vio-flight.service.template"
SERVICE_NAME="vio-flight.service"
